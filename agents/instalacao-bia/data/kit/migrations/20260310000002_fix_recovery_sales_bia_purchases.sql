-- Fix: Replace UTM-based recovery sales with bia_purchases table
-- Old logic: utm_source IS NULL OR utm_source != 'FB' (unreliable)
-- New logic: query bia_purchases directly (populated by n8n "Tag Venda Bia" node)
-- bia_purchases has: attribution (bia_direct/bia_assisted), value_cents, purchased_at, status
...
BEGIN
  SELECT json_build_object(
    ...
    'recovery', (
      SELECT json_build_object(
        -- Contact tracking (from v_recovery_dashboard)
        ...
        -- Recovery sales from bia_purchases — all time
        'recovery_sales', COALESCE(bp.total_sales, 0),
        'recovery_revenue', COALESCE(bp.total_revenue, 0),
        -- Recovery sales YESTERDAY
        'recovery_sales_yesterday', COALESCE(bp.yesterday_sales, 0),
        'recovery_revenue_yesterday', COALESCE(bp.yesterday_revenue, 0),
        -- Recovery sales TODAY
        'recovery_sales_today', COALESCE(bp.today_sales, 0),
        'recovery_revenue_today', COALESCE(bp.today_revenue, 0)
      )
      FROM
        (SELECT
        ...
        (SELECT
          ...
          ROUND(COALESCE(SUM(value_cents), 0) / 100.0, 2) AS total_revenue,
          COUNT(*) FILTER (WHERE (purchased_at AT TIME ZONE 'America/Sao_Paulo')::date = v_yesterday) AS yesterday_sales,
          ROUND(COALESCE(SUM(value_cents) FILTER (WHERE (purchased_at AT TIME ZONE 'America/Sao_Paulo')::date = v_yesterday), 0) / 100.0, 2) AS yesterday_revenue,
          COUNT(*) FILTER (WHERE (purchased_at AT TIME ZONE 'America/Sao_Paulo')::date = v_today) AS today_sales,
          ROUND(COALESCE(SUM(value_cents) FILTER (WHERE (purchased_at AT TIME ZONE 'America/Sao_Paulo')::date = v_today), 0) / 100.0, 2) AS today_revenue
        FROM bia_purchases
        WHERE status = 'approved'
          AND attribution IN ('bia_direct', 'bia_assisted')) bp
    )
  ) INTO result;
