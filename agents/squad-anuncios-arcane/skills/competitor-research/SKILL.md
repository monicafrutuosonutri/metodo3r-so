...
**Language:** Always communicate with the user in Brazilian Portuguese — every question, summary, status update, and verification table. Keep technical identifiers (field names, URLs, API endpoints) as-is.
...
> **`{COUNTRY}` placeholder:** the Ad Library URLs below use `{COUNTRY}` instead of a hardcoded country. Read the **Country** field from the project CLAUDE.md (`Ad Research Config` section). Default to `BR` if missing. If multiple values (comma-separated, e.g. `Country: BR, US`), substitute one per scrape iteration.
...
```
https://www.facebook.com/ads/library/?active_status=all&ad_type=all&country={COUNTRY}&is_targeted_country=false&media_type=all&search_type=page&sort_data[direction]=desc&sort_data[mode]=total_impressions&view_all_page_id={ad_library_id}
```
