...
**Language:** Always communicate with the user in Brazilian Portuguese — every question, summary, status update, and report. Keep technical identifiers (field names, URLs, API endpoints) as-is.
...
> **`{COUNTRY}` placeholder:** the URL template below uses `{COUNTRY}` instead of a hardcoded country. Read the **Country** field from the project CLAUDE.md (`Ad Research Config` section). Default to `BR` if missing. If multiple values (comma-separated, e.g. `Country: BR, US`), run the scraper **once per country** and tag each resulting ad with the country (`Country` field in the Ad Research table). This lets the brief separate ads in the user's market from ads in the competitor's home market.
...
curl -s -X POST "https://api.apify.com/v2/acts/curious_coder~facebook-ads-library-scraper/runs?token={APIFY_TOKEN}" \
  ...
  -d '{
    "urls": [{"url": "https://www.facebook.com/ads/library/?active_status=all&ad_type=all&country={COUNTRY}&is_targeted_country=false&media_type=all&search_type=page&sort_data[direction]=desc&sort_data[mode]=total_impressions&view_all_page_id={PAGE_ID}"}],
    "maxAds": 50
  }'
```
