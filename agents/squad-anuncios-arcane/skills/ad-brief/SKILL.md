...
**Language:** Always communicate with the user in Brazilian Portuguese — every question, status update, and the **final brief document itself** (all 8 sections). Keep technical identifiers (field names, table IDs, URLs) and exact hook quotes from the data as-is.
