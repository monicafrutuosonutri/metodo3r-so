...
Checklist:
  ...
  - "Identidade regulatória verificada preservada ou resolvida no destino"
...
curl -s "https://graph.facebook.com/${META_API_VERSION}/${ADSET_ID}?fields=id,name,campaign_id,status,optimization_goal,destination_type,bid_strategy,bid_amount,daily_budget,billing_event,promoted_object,attribution_spec,targeting,regional_regulated_categories,regional_regulation_identities,dsa_beneficiary,dsa_payor&access_token=${META_TOKEN}" > /tmp/adset.json
...
| `regional_regulated_categories` | MANTÉM; em BR exige BRAZIL_REGULATION + VOLUNTARY_VERIFICATION |
| `regional_regulation_identities` | MANTÉM somente se bater com destino/preview; senão resolver IDs verificados corretos |
| `dsa_beneficiary` / `dsa_payor` | Legado; nunca usar como substituto dos IDs |
...
- [ ] `regional_regulation_identities` preenchido e igual ao preview/registry
- [ ] Categorias BR preservadas
- [ ] `validate_only` passou e readback confirmou os IDs antes da ativação
